...
table "users" {
  ...
  column "password"   { type = varchar(256); null = false }
  ...
  unique { columns = [column.email] }
}

table "user_profiles" {
  schema      = schema.public
  column "user_id"    { type = uuid; null = false }
  column "full_name"  { type = varchar(128) }
  column "avatar_url" { type = varchar(512) }
...
table "user_sessions" {
  ...
  column "id"         { type = uuid; null = false }
  column "user_id"    { type = uuid; null = false }
  column "ip"         { type = varchar(64) }
  column "user_agent" { type = varchar(256) }
...
table "org_members" {
  ...
  column "updated_at" { type = timestamptz; null = false; default = sql("now()") }
  primary_key { columns = [column.id] }
  unique { columns = [column.org_id, column.user_id] }
...
table "org_invites" {
  ...
  column "created_at" { type = timestamptz; null = false; default = sql("now()") }
  primary_key { columns = [column.id] }
  unique { columns = [column.org_id, column.email] }
  index { columns = [column.org_id] }
...
table "project_members" {
  ...
  column "updated_at" { type = timestamptz; null = false; default = sql("now()") }
  primary_key { columns = [column.id] }
  unique { columns = [column.project_id, column.user_id] }
...
table "project_invites" {
  ...
  column "created_at" { type = timestamptz; null = false; default = sql("now()") }
  primary_key { columns = [column.id] }
  unique { columns = [column.project_id, column.email] }
  index { columns = [column.project_id] }
  index { columns = [column.email] }
} 