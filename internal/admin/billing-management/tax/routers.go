...
func RegisterRoutes(router fiber.Router, handler *TaxHandler, jwtSecret string) {
	...
	// Tax plugin config routes
	route.Post("/plugin-config", rbac_management.RBACMiddleware("tax", "create", nil), handler.SetTaxPluginConfig)
	route.Get("/plugin-config", handler.GetTaxPluginConfig)
