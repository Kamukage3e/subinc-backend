...
func NewTenantSettingsStore(db *sql.DB, log *logger.Logger) *TenantSettingsStore {
...
func (s *TenantSettingsStore) GetTenantSettings(ctx context.Context, tenantID string) (map[string]interface{}, error) {
	...
	err := s.DB.QueryRowContext(ctx, q, tenantID).Scan(&settingsStr)
	if err != nil {
...
func (s *TenantSettingsStore) UpdateTenantSettings(ctx context.Context, tenantID string, input map[string]interface{}) (map[string]interface{}, error) {
	...
	res, err := s.DB.ExecContext(ctx, q, string(settingsBytes), time.Now().UTC(), tenantID)
	if err != nil {
		return nil, err
	}
...
func NewTenantStore(db *sql.DB, log *logger.Logger) *TenantStore {
...
func (s *TenantStore) CreateTenant(ctx context.Context, tenant *Tenant) error {
	...
	_, err := s.DB.ExecContext(ctx, q, tenant.ID, tenant.Name, tenant.Settings, tenant.CreatedAt, tenant.UpdatedAt)
	if err != nil {
...
func (s *TenantStore) UpdateTenant(ctx context.Context, tenant *Tenant) error {
	...
	res, err := s.DB.ExecContext(ctx, q, tenant.ID, tenant.Name, tenant.Settings, tenant.UpdatedAt)
	if err != nil {
...
func (s *TenantStore) ListTenants(ctx context.Context) ([]interface{}, error) {
	...
	if err := rows.Err(); err != nil {
		s.log.Error("error iterating tenant rows", logger.ErrorField(err))
...
func (s *TenantStore) SearchTenants(ctx context.Context, filter TenantFilter) ([]interface{}, int, error) {
	...
	row := s.DB.QueryRowContext(ctx, countQ, args[:arg-1]...)
	var total int
