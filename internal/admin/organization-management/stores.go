...
func (s *PostgresStore) AddMember(ctx context.Context, member OrgMember) (OrgMember, error) {
	...
	id := uuid.NewString()
	now := time.Now().UTC()
	row := s.db.QueryRow(ctx, q, id, member.OrgID, member.UserID, member.Role, member.Status, member.InvitedBy, now, now)
	var out OrgMember
...
func (s *PostgresStore) UpdateMemberRole(ctx context.Context, id, role string) error {
	...
	_, err := s.db.Exec(ctx, q, id, role, time.Now().UTC())
	if err != nil {
...
func (s *PostgresStore) ListMembers(ctx context.Context, orgID string, page, pageSize int) ([]OrgMember, error) {
	...
	offset := (page - 1) * pageSize
	rows, err := s.db.Query(ctx, q, orgID, pageSize, offset)
	if err != nil {
...
func (s *PostgresStore) UpdateMember(ctx context.Context, member OrgMember) (OrgMember, error) {
	...
	row := s.db.QueryRow(ctx, q, member.ID, member.OrgID, member.UserID, member.Role, member.Status, member.InvitedBy, time.Now().UTC())
	var out OrgMember
	if err := row.Scan(&out.ID, &out.OrgID, &out.UserID, &out.Role, &out.Status, &out.InvitedBy, &out.CreatedAt, &out.UpdatedAt); err != nil {
...
func (s *PostgresStore) CreateInvite(ctx context.Context, invite OrgInvite) (OrgInvite, error) {
	...
	id := uuid.NewString()
	now := time.Now().UTC()
	row := s.db.QueryRow(ctx, q, id, invite.OrgID, invite.Email, invite.Role, invite.Status, invite.Token, invite.ExpiresAt, now)
	var out OrgInvite
